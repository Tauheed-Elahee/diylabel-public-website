/*
  # Update business hours to use 4-digit numbers

  1. Schema Changes
    - Update business_hours field to use proper numeric time format
    - Times stored as 4-digit integers (0900, 1730, 2230)
    - Closed days stored as string "closed"
    - Add validation for proper time ranges and minute values

  2. Time Format
    - Open/close times: 4-digit integers (0000-2359)
    - Valid minutes: 00, 15, 30, 45 (quarter-hour increments)
    - Closed days: string "closed"
    - Example: { "open": 900, "close": 1700 } or "closed"

  3. Validation
    - Time range: 0000-2359
    - Valid minutes: 00, 15, 30, 45
    - Logical validation: open time < close time (for same day)
*/

-- Drop existing table to recreate with updated time format
DROP TABLE IF EXISTS partnership_applications CASCADE;

-- Create updated partnership applications table with numeric time format
...
-- Create function to validate business hours format
...
BEGIN
  ...
  FOR day_name IN SELECT unnest(ARRAY['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday']) LOOP
    ...
    -- If day is closed, it should be the string "closed"
    IF day_data = '"closed"'::jsonb THEN
      CONTINUE;
    END IF;
    
    -- If day is open, it should have open and close times as numbers
    IF jsonb_typeof(day_data) = 'object' THEN
      ...
      -- Validate minute values (00, 15, 30, 45)
      IF (open_time % 100) NOT IN (0, 15, 30, 45) OR (close_time % 100) NOT IN (0, 15, 30, 45) THEN
        RETURN false;
      END IF;
      ...
      END IF;
    ELSE
      -- Day data is neither "closed" nor a valid object
      RETURN false;
    END IF;
  END LOOP;
...
CHECK (business_hours IS NULL OR validate_business_hours(business_hours));
